...
const KitchenDisplay = () => {
    ...
    try {
      ...
      for (const item of formattedItems) {
        const {
          data: toppings,
          error: toppingsError
        } = await supabase.from('order_item_toppings').select(`
            id,
            order_item_id,
            topping_id,
            price,
  ...
  useEffect(() => {
    const channel = supabase.channel('kds-orders-channel').on('postgres_changes', {
      event: '*',
      schema: 'public',
      table: 'orders'
    }, payload => {
  ...
  const renderColumns = () => {
    return <DragDropContext onDragEnd={onDragEnd}>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 h-[calc(100vh-180px)]">
          {Object.entries(columns).map(([columnId, columnOrders]) => <div key={columnId} className="flex flex-col h-full">
              <div className={`mb-2 rounded-md p-2 font-semibold text-white ${columnId === 'New' ? 'bg-purple-600' : columnId === 'In Progress' ? 'bg-amber-600' : 'bg-green-600'} flex items-center justify-between`}>
              ...
              <Droppable droppableId={columnId}>
                {provided => <div {...provided.droppableProps} ref={provided.innerRef} className="flex-1 bg-gray-50 rounded-md p-2 overflow-hidden">
                    <ScrollArea className="h-full pr-2">
                      <div className="space-y-3 min-h-full">
                        {columnOrders.length === 0 ? <div className="h-32 flex items-center justify-center text-muted-foreground text-sm border border-dashed rounded-md">
                            No orders in this column
                          </div> : columnOrders.map((order, index) => <Draggable key={order.id.toString()} draggableId={order.id.toString()} index={index}>
                              {provided => <div ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps} onClick={() => handleOpenOrderDetails(order)} className="border bg-card shadow-sm rounded-md p-3 hover:shadow-md cursor-pointer transition-shadow">
                                  <div className="flex justify-between items-start mb-2">
                                    <div>
                                      <p className="font-semibold text-lg">Order #{order.id}</p>
                                      <p className="text-sm text-muted-foreground">
                                        {order.customer_type === 'Table' ? `Table #${order.table_number}` : 'Takeaway'}
                                      </p>
                                    </div>
        ...
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-hidden">
          <DialogHeader>
            <DialogTitle className="flex justify-between items-center">
              ...
              <Badge variant={selectedOrder.status === 'Completed' ? 'default' : selectedOrder.status === 'In Progress' ? 'secondary' : 'outline'}>
                {selectedOrder.status}
              </Badge>
            </DialogTitle>
            <DialogDescription>
              {selectedOrder.customer_type === 'Table' ? `Table #${selectedOrder.table_number}` : 'Takeaway'} • {format(new Date(selectedOrder.created_at), 'MMM d, h:mm a')}
            </DialogDescription>
          </DialogHeader>
          ...
          <ScrollArea className="max-h-[60vh]">
            <div className="space-y-4 p-1">
                  ...
                  <CardContent className="p-3 pt-1">
                    <div className="space-y-1">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span>{formatDistance(new Date(selectedOrder.created_at), new Date(), {
                          addSuffix: true
                        })}</span>
                      </div>
                    </div>
                ...
                <Card>
                  ...
                  <CardContent className="p-3 pt-1">
                    <div className="space-y-1">
                      ...
                      {selectedOrder.customer_type === 'Table' && <div className="flex justify-between">
              ...
              <Card>
                ...
                <CardContent className="p-3 pt-1">
                  {isLoadingDetails ? <div className="flex justify-center py-8">
                      <div className="animate-spin w-6 h-6 border-2 border-primary border-t-transparent rounded-full"></div>
                    </div> : orderDetails.length === 0 ? <div className="text-center py-4 text-muted-foreground">
                      No items found for this order
                    </div> : <div className="space-y-3">
                      {orderDetails.map(item => <div key={item.id} className="border rounded-md p-3">
                          <div className="flex justify-between items-start">
                            <div className="space-y-1">
                              ...
                              {item.notes && <p className="text-sm italic bg-muted p-1 rounded-sm mt-1">
                                  Note: {item.notes}
                                </p>}
                            </div>
                            <p className="font-medium">
                          ...
                          {item.toppings && item.toppings.length > 0 && <div className="mt-2 border-t pt-2">
                              <p className="text-xs text-muted-foreground mb-1">Toppings:</p>
                              <div className="pl-2 space-y-1">
                                {item.toppings.map(topping => <div key={topping.id} className="flex justify-between text-sm">
          ...
          <div className="flex justify-between mt-2">
            ...
            <div className="flex gap-2">
              {selectedOrder.status === 'New' && <Button onClick={async () => {
              try {
                const {
                  error
                } = await supabase.from('orders').update({
              ...
              {selectedOrder.status !== 'In Progress' && selectedOrder.status !== 'New' && <Button onClick={async () => {
              try {
                const {
                  error
                } = await supabase.from('orders').update({
              ...
              {selectedOrder.status !== 'Completed' && <Button onClick={async () => {
              try {
                const {
                  error
                } = await supabase.from('orders').update({
              ...
              {selectedOrder.status !== 'Cancelled' && <Button variant="destructive" onClick={async () => {
              try {
                const {
                  error
                } = await supabase.from('orders').update({
  ...
  return <AdminLayout>
      <div className="space-y-6 h-full flex flex-col">
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-2">
            <Button variant="outline" onClick={() => refetch()} className="text-left rounded-lg font-normal">
              Refresh
            </Button>
          </div>
        </div>
        ...
        {isLoading ? <div className="flex-1 flex items-center justify-center">
          ...
          </div> : isError ? <div className="flex-1 flex items-center justify-center">
