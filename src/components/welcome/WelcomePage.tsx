...
const WelcomePage: React.FC<WelcomePageProps> = ({
    ...
    const fetchOrderingSettings = async () => {
      try {
        const {
          data,
          error
        } = await supabase.from('settings').select('*').eq('key', 'ordering_settings').maybeSingle();
        if (error) {
  ...
  return <div className="relative h-full w-full flex flex-col items-center justify-center bg-gradient-to-b from-gray-50 to-gray-100">
      <AnimatePresence mode="wait">
        {!showTableSelector ? <motion.div key="welcome" initial={{
        opacity: 0,
        y: 20
      }} animate={{
        opacity: 1,
        ...
        duration: 0.4
          ...
          delay: 0.4
        }} className="text-3xl font-semibold mb-10 text-gray-800 text-center mx-px my-[25px]">
              Comment souhaitez-vous déguster votre repas aujourd'hui ?
            </motion.h2>
            
            <div className="flex flex-col sm:flex-row gap-6 justify-center">
              <Button size="lg" onClick={handleTakeaway} className="text-xl py-8 sm:min-w-[220px]">
                À emporter
              </Button>
              <Button size="lg" onClick={handleEatIn} className="text-xl py-8 sm:min-w-[220px]">
                Sur place
              </Button>
            </div>
          </motion.div> : <motion.div key="table-selector" initial={{
        opacity: 0,
        y: 20
      }} animate={{
        opacity: 1,
        ...
        y: -20
            ...
            <TableSelector onSelectTable={handleTableSelected} onCancel={() => setShowTableSelector(false)} />
          </motion.div>}
      </AnimatePresence>
    </div>;
};
